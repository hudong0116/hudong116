...
(function(window, undefined){
	var ADSupportsTouches = ("createTouch" in document) || ('ontouchstart' in window) || 0,
		...
		TouchSlider=function(opt){
			...
			try{
				if(this.container.nodeName.toLowerCase()=='dl'){
					this.element=this.container;
					this.container=this.element.parentNode;
				}else{
					this.element=this.container.getElementsByTagName('dl')[0];
				}
				if(typeof this.element==='undefined')throw new Error('Can\'t find "dl"');
	...
	TouchSlider.prototype={
		...
		setup: function(){
			...
			this.slides=this.opt.className?this.$E(this.opt.className,'dd',this.element):this.element.getElementsByTagName('dd');
		...
		prev: function (offset) {
			...
			page = 1;
			if (index > 0) {
				listname = $("#pagenavi1").find("dd").eq(index).attr("data-listname");
				classname = $("#pagenavi1").find("dd").eq(index).attr("data-classname");
				$("#" + listname).html("");
				getnewslist(listname, classname, page, size);
			}
			else {
				listname = "list_column_0102";
				classname = "column_0102";
				$("#" + listname).html("");
				getnewslist0103(page0102, size);
			}
			document.body.scrollTop = document.documentElement.scrollTop = 0;
		},
		next: function (offset) {
			if (typeof offset == 'undefined') offset = 1;
			var idx = (this.index + offset) % this.length;
			this.slide(idx);
			//数据初始化			
			page0102 = 1;
			page = 1;
			if (idx > 0) {
				listname = $("#pagenavi1").find("dd").eq(idx).attr("data-listname");
				classname = $("#pagenavi1").find("dd").eq(idx).attr("data-classname");
			}
			else {
				listname = "list_column_0102";
				classname = "column_0102";
